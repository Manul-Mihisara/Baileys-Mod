...
const extractNewsletterMetadata = (node, isCreate) => {
  try {
    ...
    if (!result) throw new Error("No result content found")

    const parsed = JSON.parse(result)
    const metadataPath = parsed?.data?.[isCreate ? Types_1.XWAPaths.CREATE : Types_1.XWAPaths.NEWSLETTER]

    if (!metadataPath) throw new Error("No metadataPath found in newsletter response")
    ...
    console.error("extractNewsletterMetadata error:", err.message)
    return null
  }
}
